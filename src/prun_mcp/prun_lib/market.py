...
class MarketError(Exception):
    """Base error for market operations."""

    pass


class InvalidDirectionError(MarketError):
    """Invalid direction specified."""

    def __init__(self, direction: str) -> None:
        self.direction = direction
        super().__init__(f"Invalid direction: {direction}. Must be 'buy' or 'sell'.")


class InvalidQuantityError(MarketError):
...
class InvalidSideError(MarketError):
    """Invalid side specified."""

    def __init__(self, side: str) -> None:
        self.side = side
        super().__init__(f"Invalid side: {side}. Must be 'buy', 'sell', or 'both'.")


class InvalidLevelsError(MarketError):
    """Invalid levels specified."""

    def __init__(self, levels: int) -> None:
        self.levels = levels
        super().__init__(f"Invalid levels: {levels}. Must be 1-100.")


class InvalidDaysError(MarketError):
    """Invalid days specified."""

    def __init__(self, days: int) -> None:
        self.days = days
        super().__init__(f"Invalid days: {days}. Must be 1-30.")
...
def generate_market_warnings(
    ...
    if mm_sell is not None and ask is not None:
        ...
        if ask >= ceiling_threshold:
            warnings.append(
                f"{prefix}Price near MM ceiling ({format_number(ask)} vs "
                f"{format_number(mm_sell)}) — limited upside"
            )
    ...
    if mm_buy is not None and bid is not None:
        ...
        if bid <= floor_threshold:
            warnings.append(
                f"{prefix}Price near MM floor ({format_number(bid)} vs "
                f"{format_number(mm_buy)}) — limited downside"
            )

    return warnings


def walk_order_book(orders: list[dict[str, Any]], quantity: int) -> dict[str, Any]:
    ...
    Returns:
        Dict with fill information including can_fill, vwap, slippage, etc.
    """
    fills = []
    cumulative_units = 0
    cumulative_cost = 0.0
...
def generate_fill_recommendations(
...
def calculate_price_stats(candles: list[dict[str, Any]]) -> dict[str, Any]:
...
def format_market_summary_section(
    ...
    bid_str = format_number(bid) if bid is not None else "N/A"
    ask_str = format_number(ask) if ask is not None else "N/A"
    ...
    # Bid/Ask line
    bid_part = f"Bid: {bid_str} ({format_number(bid_depth)} units)"
    ask_part = f"Ask: {ask_str} ({format_number(ask_depth)} units)"
    ...
    if bid is not None and ask is not None:
        ...
        mid = (bid + ask) / 2
        lines.append(
            f"Spread: {format_number(spread)} ({spread_pct:.1f}%) | "
            f"Mid: {format_number(mid)}"
        )
    else:
    ...
    if demand > 0:
        ...
        lines.append(
            f"Supply: {format_number(supply)} | Demand: {format_number(demand)} | "
    ...
    if mm_buy is not None or mm_sell is not None:
        ...
        if mm_buy is not None:
            mm_parts.append(f"Buy {format_number(mm_buy)}")
        if mm_sell is not None:
            mm_parts.append(f"Sell {format_number(mm_sell)}")
        lines.append(f"MM: {' | '.join(mm_parts)}")

    return "\n".join(lines)


# Implementation constants for async functions
MULTI_TICKER_LEVEL_CAP = 10  # Levels per ticker when multiple requested


async def get_market_summary_async(ticker: str, exchange: str) -> str:
    ...
    Returns:
        Plain text market summary with bid/ask, spread, supply/demand, and warnings.
    ...
    for t, data in results:
        ...
        section = format_market_summary_section(t, exchange, data)
        sections.append(section)

        warnings = generate_market_warnings(data, t if is_multi else None)
        all_warnings.extend(warnings)

    if not sections and not_found:
        return f"No exchange data for: {', '.join(not_found)}"
...
async def analyze_fill_cost_async(
    ...
    Returns:
        Fill analysis including fills breakdown, VWAP, slippage.

    Raises:
        InvalidExchangeError: If exchange code is invalid.
        InvalidDirectionError: If direction is invalid.
    ...
    try:
        ...
        return {"error": f"No exchange data for {ticker}.{exchange}"}

    # Get the relevant order book side
    if direction == "buy":
        raw_orders = data.get("SellingOrders", [])
        orders = aggregate_orders_by_price(raw_orders, descending=False)
    else:
        raw_orders = data.get("BuyingOrders", [])
        orders = aggregate_orders_by_price(raw_orders, descending=True)

    if not orders:
        side = "sell" if direction == "buy" else "buy"
        return {"error": f"No {side} orders available for {ticker}.{exchange}"}

    # Walk the order book
    fill_result = walk_order_book(orders, quantity)
    ...
    recommendations = generate_fill_recommendations(fill_result["fills"], quantity)
    ...
    if not fill_result["can_fill"]:
        ...
        if fill_result["fill_quantity"] >= total_available:
            warnings.append(
                f"Full order would clear entire "
                f"{'sell' if direction == 'buy' else 'buy'} side"
            )
...
async def get_order_book_depth_async(
    ticker: str, exchange: str, side: str = "both", levels: int = 20
) -> dict[str, Any]:
    """Get full order book with aggregated levels and cumulative calculations.

    Args:
        ticker: Material ticker symbol(s). Single or comma-separated.
        exchange: Exchange code.
        side: "buy" (bids), "sell" (asks), or "both".
        levels: Maximum price levels per side (capped to 10 for multi-ticker).

    Returns:
        Order book data with aggregated levels and cumulative calculations.

    Raises:
        InvalidExchangeError: If exchange code is invalid.
        InvalidSideError: If side is invalid.
        InvalidLevelsError: If levels is invalid.
    """
    from prun_mcp.fio import FIONotFoundError, get_fio_client
    ...
    if levels < 1 or levels > 100:
        raise InvalidLevelsError(levels)
    ...
    for t, data in results:
        ...
        if side in ("sell", "both"):
            sell_orders = aggregate_orders_by_price(
                data.get("SellingOrders", []), descending=False
            )
            book["sell_orders"] = build_order_book_levels(
                sell_orders, effective_levels, is_sell=True
            )

        # Build buy side (bids)
        if side in ("buy", "both"):
            buy_orders = aggregate_orders_by_price(
                data.get("BuyingOrders", []), descending=True
            )
            book["buy_orders"] = build_order_book_levels(
    ...
    if not order_books and not_found:
        return {"error": f"No exchange data for: {', '.join(not_found)}"}
...
async def get_price_history_async(
    ...
    """Get historical price data with OHLCV and summary statistics.

    Args:
        ticker: Material ticker symbol(s). Single or comma-separated.
        exchange: Exchange code.
        days: Historical lookback period in days (1-30).

    Returns:
        Historical data with daily OHLCV and summary statistics.

    Raises:
        InvalidExchangeError: If exchange code is invalid.
        InvalidDaysError: If days is invalid.
    """
    from prun_mcp.fio import FIONotFoundError, get_fio_client
    ...
    for t, data in results:
        ...
        stats = calculate_price_stats(daily_candles)
    ...
    if not histories and not_found:
        return {"error": f"No price history for: {', '.join(not_found)}"}
...
async def get_price_history_summary_async(
    ticker: str, exchange: str, days: int = 7
) -> str:
    ...
    Returns:
        Plain text historical comparison with insights.
    ...
    for t, current, history in results:
        ...
        if daily_candles:
            ...
            # Insights
            insights = generate_history_insights(
    ...
    if not sections and not_found:
        return f"No data for: {', '.join(not_found)}"
